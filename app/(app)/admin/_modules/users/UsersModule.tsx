...
export default function UsersModule() {
  ...
  const filtered = useMemo(() => {
    ...
    if (needle) {
      list = list.filter((u) =>
        [(u.full_name ?? ""), (u.id ?? ""), (u.role ?? ""), (u.email ?? "")].some((s) =>
  ...
  async function saveAreas() {
    ...
    try {
      ...
      setError(e?.message || "Error guardando accesos.");
    } finally {
  ...
  const filteredAreas = useMemo(() => {
    ...
    return areas.filter((a) => [(a.name ?? ""), (a.type ?? "")].some((s) => s.toLowerCase().includes(needle)));
  }, [areas, areasQ]);
    ...
    <div>
      ...
      >
        <div style={{ fontSize: 13, color: "var(--muted)", fontWeight: 900 }}>
          Hotel: <span style={{ color: "var(--text)" }}>{activeHotelId ?? "—"}</span>
        </div>
        ...
        <button
          ...
          style={{ ...s.btn, background: "black", color: "white" }}
        >
          + Crear usuario
        </button>

        <button onClick={loadUsers} style={s.btn}>
          Recargar
        </button>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  {u.role === "superadmin" ? (
                    ...
                    <select
                      value={isEditableRole(u.role) ? u.role : "auditor"}
                      disabled={disabled}
                      onChange={(e) => {
          ...
          >
            ...
            <form onSubmit={createUser} style={{ marginTop: 14 }}>
              <div style={{ display: "grid", gap: 12 }}>
                {[
                  ["Nombre completo", "text", fullName, setFullName, "Opcional", false],
                  ["Email", "email", email, setEmail, "correo@hotel.com", true],
                  ["Password", "password", password, setPassword, "mínimo 8 caracteres", true],
                ...
                <div>
                  ...
                  <select
                    value={createRole}
                    onChange={(e) => setCreateRole(e.target.value as EditableRole)}
                    style={{
                      width: "100%",
                      marginTop: 6,
                      padding: 10,
                  ...
                  >
                    ...
                    <option value="engineering">Engineering 🔧 — Depto. técnico</option>
                    <option value="systems">Systems 💻 — Sistemas / IT</option>
                  </select>
                  ...
                  {createRole === "quality" && (
                    ...
                    >
                      Las auditorías de este usuario se registrarán como canal <strong>Quality</strong>,
                      separadas de las auditorías internas.
                    </div>
                  )}

                  {createRole === "engineering" && (
                    <div
                      style={{
                        marginTop: 8,
                        padding: "8px 12px",
                    ...
                    >
                      Este usuario no audita. Tendrá visibilidad sobre incidencias técnicas asignadas a{" "}
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          borderRadius: 999,
                          border: "1px solid var(--border)",
                          background: inactive ? "rgba(0,0,0,0.03)" : "rgba(0,200,0,0.10)",
                          color: inactive ? "var(--muted)" : "green",
                        }}
                      >
                        {inactive ? "INACTIVA" : "OK"}
                      </span>
                    </label>
                  );
                })
