...
export default function UserDetailPage() {
  ...
  async function loadAreasAndAccess(targetUserId: string, hotelId: string) {
    ...
    const { data: areasData, error: areasErr } = await supabase.from("areas").select("id,name,type,active").eq("hotel_id", hotelId).eq("active", true).order("name", { ascending: true });
    if (areasErr) throw areasErr;
    setAreas((areasData ?? []) as AreaRow[]);
    const { data: accessData, error: accessErr } = await supabase.from("user_area_access").select("area_id").eq("user_id", targetUserId).eq("hotel_id", hotelId);
  ...
  const deleteUser = async () => {
    if (!me || !userRow) return;
    setMsg(null);
    if (userRow.id === me.id) { setMsg("No puedes borrarte a ti mismo."); return; }
    if (deleteConfirm.trim().toUpperCase() !== "BORRAR") { setMsg("Para borrar, escribe BORRAR en el campo de confirmación."); return; }
    try {
      setDeleting(true);
      const { data: sessionData } = await supabase.auth.getSession();
      const accessToken = sessionData?.session?.access_token;
      if (!accessToken) throw new Error("Sesión inválida.");
      const res = await fetch("/api/admin/delete-user", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ user_id: userRow.id }) });
      ...
      router.push("/users");
    } catch (e: any) { setMsg(`❌ Error borrando: ${e?.message ?? "desconocido"}`); }
  ...
  if (!userRow) return (
    <main style={{ padding: 24, fontFamily: "system-ui" }}>
      ...
      <button onClick={() => router.push("/users")} style={{ marginTop: 12, padding: "10px 14px", borderRadius: 10, border: "1px solid #000", background: "#fff", cursor: "pointer", fontWeight: 800 }}>Volver</button>
    </main>
  );
    ...
    <main style={{ padding: 24, fontFamily: "system-ui" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <button onClick={() => setShowDelete((v) => !v)} style={{ padding: "10px 14px", borderRadius: 10, border: "1px solid rgba(220,20,60,0.6)", background: "rgba(220,20,60,0.08)", color: "crimson", cursor: "pointer", fontWeight: 950 }}>Borrar usuario</button>
        </div>
      </div>

      {msg && <div style={{ marginTop: 14, padding: 12, borderRadius: 12, border: msg.startsWith("✅") ? "1px solid #0a0" : "1px solid crimson", background: "#fff" }}>{msg}</div>}
        ...
        <div style={{ marginTop: 12 }}>
          {areasLoading ? <div style={{ opacity: 0.8 }}>Cargando áreas…</div> : areas.length === 0 ? <div style={{ opacity: 0.8 }}>No hay áreas activas en este hotel.</div> : (
            <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
              {areas.map((a) => (
                <label key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, borderRadius: 12, border: "1px solid rgba(0,0,0,0.18)", background: "rgba(255,255,255,0.55)", cursor: "pointer", userSelect: "none" }}>
                  <input type="checkbox" checked={selectedAreaIds.includes(a.id)} onChange={() => toggleArea(a.id)} style={{ width: 18, height: 18 }} />
