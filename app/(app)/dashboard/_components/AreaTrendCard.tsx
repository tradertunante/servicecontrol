...
export default function AreaTrendCard({
  ...
  const series = useMemo(() => {
    ...
    for (const [areaId, monthMap] of grouped) {
      ...
      if (points.length >= 2) {
        result.push({
          areaId,
          areaName: areaNameById.get(areaId) || "Área",
          points,
        });
      }
    }
  ...
  if (series.length === 0) {
    return (
      <div className="rounded-2xl border bg-white p-5">
        <h3 className="text-sm font-extrabold text-gray-900">Tendencia por área</h3>
  ...
  // Collect all month keys for x axis
