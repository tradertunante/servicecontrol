...
export default function WorstAuditsCard({
  ...
  return (
    <div style={card} className="card" data-onboarding="worst-audits">
      <div className="sectionTitle">{t("worstAudits")}</div>
      ...
      <style jsx global>{`
        ...
        .auditBadgeSlot { grid-area: badge; align-self: center; }
        .auditBadge { width: 42px; height: 42px; display: grid; place-items: center; }
        .auditBadge span { font-size: 20px; line-height: 1; }
        .auditContent { grid-area: content; min-width: 0; display: flex; flex-direction: column; justify-content: center; }
        .auditTitle { font-weight: 950; font-size: 15px; line-height: 1.3; word-break: break-word; overflow-wrap: anywhere; color: ${fg}; }
        .auditAside { grid-area: aside; display: flex; flex-direction: column; align-items: flex-end; justify-content: center; gap: 3px; min-width: 116px; text-align: right; align-self: center; justify-self: end; }
        .auditScore { font-weight: 950; font-size: 32px; line-height: 1; letter-spacing: -0.04em; white-space: nowrap; }
        .auditBtn { min-width: 104px; width: fit-content; }
        @media (max-width: 720px) {
          .auditRow { grid-template-columns: 40px minmax(0,1fr); grid-template-areas: "badge content" "aside aside"; padding: 12px 14px; row-gap: 8px; }
          .auditBadgeSlot { align-self: start; }
          .auditAside { min-width: 0; flex-direction: row; align-items: center; justify-content: space-between; gap: 8px; text-align: left; width: 100%; }
          .auditScore { font-size: 26px; }
          .auditBtn { width: auto; min-width: 112px; }
        }
