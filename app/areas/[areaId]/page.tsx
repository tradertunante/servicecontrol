...
// ✅ dedupe SIN Set (evita error de downlevelIteration en Vercel)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // ✅ IDs deduplicados SIN Set
        const runIds = uniqStrings(finalRuns.map((r) => r.id));
        const templateIds = uniqStrings(finalRuns.map((r) => r.audit_template_id));
        const executorIds = uniqStrings(
          finalRuns.map((r) => r.executed_by).filter(Boolean) as string[]
        );
        ...
        if (runIds.length) {
          ...
          const questionIds = uniqStrings(answers.map((a) => a.question_id));
