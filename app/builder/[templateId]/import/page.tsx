...
// Normaliza “visualmente igual” (espacios raros, dobles, mayúsculas)
...
// ✅ Soporta pegado con TAB o con espacios (2+ espacios)
// - Si hay tabs -> TSV
// - Si NO hay tabs -> split por 2+ espacios (lo típico cuando copias de PDF/Word o ciertas tablas)
...
function normalizeStdAndTag(standardRaw: string, tagRaw: string) {
  ...
  // Si standard viene como "[Facility] bla bla", extraemos tag
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (tErr || !data) setError(tErr?.message ?? "No se encontró la auditoría.");
  ...
  const parsed = useMemo(() => {
    ...
    if (lines.length < 2) {
      return { rows: [] as ParsedRow[], sectionsCount: 0, questionsCount: 0, parseError: "Pega la tabla con cabeceras y al menos 1 fila." };
    }
  ...
  async function importNow() {
    ...
    try {
      ...
      // 3) Validación dura: comprobar que TODAS las filas resuelven sección
      ...
      // 4) Insertar preguntas con orden exacto por sección
  ...
  return (
    <main style={{ padding: 24 }}>
      <BackButton fallback={`/builder/${templateId}`} />

      <h1 style={{ fontSize: 44, marginBottom: 6 }}>Importar Excel (copiar/pegar)</h1>
