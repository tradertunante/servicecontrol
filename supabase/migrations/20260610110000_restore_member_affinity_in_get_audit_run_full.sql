-- Fix: restore member_affinity in get_audit_run_full team members payload.
-- The 20260606110000 (photo_paths) and 20260607160000 (employee_name) fixes
-- recreated the function from a pre-20260602 base that returned _outOfArea,
-- dropping the member_affinity field and the audit_template_members logic.
-- The audit collaborator <select> groups options by member_affinity
-- ('template' / 'area' / 'other'), so it rendered empty.
-- This version = 20260607160000 (employee_name + photo_paths) + sections
-- 9/10 from 20260602000000 (member affinity).
...
begin
  ...
  select id, status, score, notes, room_number, employee_name, executed_at, executed_by,
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'photo_paths', coalesce(a.photo_paths, '{}')
    )
  ), '[]'::jsonb)
    into v_answers
  ...
  return jsonb_build_object(
    ...
    'data', jsonb_build_object(
      'run', jsonb_build_object(
        ...
        'employee_name', v_run.employee_name,
