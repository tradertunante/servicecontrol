-- Fix: when an admin/GM/QM submits an audit, the auto-created reaudit was assigned
-- back to that elevated user. Instead, look for an active manager with area access.
-- Roles considered "elevated" (should not receive reaudit): admin, general_manager,
-- quality, superadmin. Fallback: keep original assignee if no manager found.
...
begin
  ...
  begin
    ...
    if coalesce(v_actor.role, '') not in ('superadmin', 'admin', 'general_manager', 'manager', 'auditor', 'quality') then
      return jsonb_build_object(
        'ok', false,
        'code', 'FORBIDDEN_ROLE',
    ...
    if v_should_create_reaudit then
      ...
      if found then
        ...
        -- Resolve initial assignee from the original run
        v_reaudit_assigned_auditor_id := coalesce(v_run.assigned_auditor_id, v_run.executed_by);

        -- If the assignee is an elevated role (admin/GM/QM/superadmin), redirect to
        -- the area manager so the reaudit lands on the right person, not the auditor.
        select role into v_assignee_role
        from public.profiles
        where id = v_reaudit_assigned_auditor_id;

        if coalesce(v_assignee_role, '') in ('admin', 'general_manager', 'quality', 'superadmin') then
          select uaa.user_id into v_area_manager_id
          from public.user_area_access uaa
          join public.profiles p on p.id = uaa.user_id
          where uaa.hotel_id = v_run.hotel_id
    ...
    loop
      ...
      if coalesce(v_q.owner_department, '') in ('it', 'systems', 'engineering') then
        ...
        values (
          ...
          v_q.id,
          case
            when v_q.owner_department = 'systems' then 'it'
            else v_q.owner_department
          end,
          v_q.text,
          'open',
          null,
          false,
          coalesce(v_run.executed_at, now()),
          now()
        )
