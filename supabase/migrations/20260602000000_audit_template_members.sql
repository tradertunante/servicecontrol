-- Junction table: links team members to specific audit templates
-- Members linked here appear first ("Equipo habitual") in the auditor picker.

create table public.audit_template_members (
  id               uuid primary key default gen_random_uuid(),
  audit_template_id uuid not null references public.audit_templates(id) on delete cascade,
  team_member_id    uuid not null references public.team_members(id)    on delete cascade,
  created_at        timestamptz not null default now(),
  unique (audit_template_id, team_member_id)
);

alter table public.audit_template_members enable row level security;

-- All reads/writes go through supabaseAdmin or security-definer RPCs; deny direct client access.

-- ─────────────────────────────────────────────────────────────────────────────
-- Update get_audit_run_full to return member_affinity instead of _outOfArea.
-- Values: 'template' → assigned to this template (shown first)
--         'area'     → linked to the area but not this template
--         'other'    → neither (only the currently-saved member, as fallback)
-- ─────────────────────────────────────────────────────────────────────────────
...
declare
  ...
  v_template_linked_ids uuid[];
  v_area_linked_ids     uuid[];
begin
  -- 1. Load actor
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'id', s.id, 'name', s.name, 'active', s.active, 'created_at', s.created_at
  ...
  -- 9. Member affinity sets
  select array_agg(distinct atm.team_member_id)
    into v_template_linked_ids
    from public.audit_template_members atm
   where atm.audit_template_id = v_run.audit_template_id;

  select array_agg(distinct tma.team_member_id)
    into v_area_linked_ids
    from public.team_member_areas tma
   where tma.area_id = v_run.area_id;

  -- 10. Team members — sorted template → area → other, then alpha within each group
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'id', tm.id,
      'full_name', tm.full_name,
      'member_affinity', case
        when tm.id = any(coalesce(v_template_linked_ids, array[]::uuid[])) then 'template'
        when tm.id = any(coalesce(v_area_linked_ids,     array[]::uuid[])) then 'area'
        else 'other'
      end
    ) order by
      case
        when tm.id = any(coalesce(v_template_linked_ids, array[]::uuid[])) then 1
        when tm.id = any(coalesce(v_area_linked_ids,     array[]::uuid[])) then 2
        else 3
      end asc,
      tm.full_name asc
  ), '[]'::jsonb)
   ...
   where tm.active = true
     and (
       tm.id = any(coalesce(v_template_linked_ids, array[]::uuid[]))
       or tm.id = any(coalesce(v_area_linked_ids,  array[]::uuid[]))
  ...
  return jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'area', jsonb_build_object(
        'id', v_area.id, 'name', v_area.name, 'type', v_area.type, 'hotel_id', v_area.hotel_id
      ),
