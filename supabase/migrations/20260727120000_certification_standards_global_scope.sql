-- ============================================================================
-- Certificados de alcance GLOBAL (catálogo compartido) además de por-hotel
-- ============================================================================
-- Motivo: "Paradisus Standards" (Forbes+LHW+Meliá fusionados) es un PACK
-- GLOBAL (audit_templates.scope='global', hotel_id NULL) editado en
-- /superadmin/templates/[id], no una plantilla de un hotel concreto. El
-- primer diseño (20260727100000) solo soportaba certificados por hotel y no
-- cubría este caso. Este migration permite hotel_id NULL en
-- certification_standards para representar el catálogo global (Forbes, LHW,
-- Meliá como marca corporativa), gestionable solo por superadmin, siguiendo
-- el mismo patrón que global_audit_packs / standard_libraries
-- (sc_can_view_global_library_assets / sc_can_manage_global_library_assets,
-- ver 20260316190000).
-- ============================================================================

alter table public.certification_standards
  alter column hotel_id drop not null;

-- La unique index anterior (hotel_id, lower(name)) no distingue múltiples
-- filas globales con hotel_id NULL entre sí (NULL <> NULL en un índice
-- normal), así que se reemplaza por dos índices parciales.
drop index if exists certification_standards_hotel_name_uniq;
...
create unique index if not exists certification_standards_global_name_uniq
  on public.certification_standards (lower(btrim(name)))
  where hotel_id is null;

-- ── certification_standards: soporta filas globales (hotel_id null) ────────
...
to authenticated
using (
  (hotel_id is not null and public.sc_is_same_hotel(hotel_id))
  or (hotel_id is null and public.sc_can_view_global_library_assets())
);
...
to authenticated
with check (
  (hotel_id is not null and public.sc_has_hotel_role(hotel_id, array['superadmin', 'admin']))
  or (hotel_id is null and public.sc_can_manage_global_library_assets())
);
...
-- ── audit_question_certifications: permitir global↔global y corregir el
--    matching de hotel (NULL = NULL es NULL en SQL, no true) ───────────────
...
with check (
  exists (
    ...
    where q.id = audit_question_certifications.question_id
      and (
        (t.hotel_id is null and public.sc_can_manage_global_library_assets())
        or (t.hotel_id is not null and public.sc_has_hotel_role(t.hotel_id, array['superadmin', 'admin']))
      )
  )
  and exists (
    select 1
    ...
    where cs.id = audit_question_certifications.certification_standard_id
      and cs.hotel_id is not distinct from t.hotel_id
  )
);
...
-- ── RPC: certificados por el hotel/alcance de la PLANTILLA, no del run ─────
-- Antes filtraba cs.hotel_id = v_run.hotel_id, lo que nunca casaba con
-- certificados globales (hotel_id null) en auditorías de una plantilla
-- global asignada directamente a un hotel. Ahora usa "is not distinct from"
-- contra el hotel_id de la propia plantilla (null si es global).
...
begin
  ...
  select hotel_id into v_template_hotel_id
  ...
  select coalesce(jsonb_agg(
   ...
   where cs.hotel_id is not distinct from v_template_hotel_id
     and cs.active = true
     and counts.total > 0;
