-- Add team-level template progress to rpc_team_summary
-- Purpose:
-- expose team coverage by rubric/template without depending on per-auditor assignment splits.
-- Leaderboard semantics stay per auditor; template_progress is team-level by template.
...
begin
  ...
  template_progress_rows as (
    select
      abt.template,
      abt.target,
      least(coalesce(trt.completed_count, 0), abt.target)::int as completed,
      greatest(abt.target - coalesce(trt.completed_count, 0), 0)::int as remaining,
      case
        when abt.target > 0
          then round((least(coalesce(trt.completed_count, 0), abt.target)::numeric / abt.target::numeric) * 100, 2)
        else 0
      end as progress_pct
