...
- `POST /api/reaudits/actions`, `POST /api/corrective-actions/status` y `POST /api/admin/user-area-access/set` ahora delegan sus mutaciones multi-tabla a RPCs transaccionales
- lifecycle de usuarios endurecido moviendo la sincronización de `profiles`/`user_area_access` a triggers sobre `auth.users`, de modo que create/delete desde Supabase Auth no deje estado local parcial
